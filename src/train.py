...
class GANTrainer:
    def __init__(self, config):
        ...
        # Loss functions
        self.criterion_GAN = nn.BCEWithLogitsLoss()  # For adversarial loss
        self.criterion_L1 = nn.L1Loss()  # For reconstruction loss
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (low_imgs, high_imgs) in enumerate(pbar):
            ...
            # Labels for real and fake (match discriminator output shape)
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for low_imgs, high_imgs in self.val_loader:
                ...
                if low_imgs is self.val_loader.dataset[0][0]:
                    self.writer.add_images('Validation/Low', (low_imgs + 1) / 2, epoch)
                    self.writer.add_images('Validation/Generated', (fake_imgs + 1) / 2, epoch)
                    self.writer.add_images('Validation/High', (high_imgs + 1) / 2, epoch)
